<details class="relative">
    <summary class="list-none flex items-center gap-2 cursor-pointer select-none text-sm font-semibold text-muted">
        {{ auth()->user()->name }}
        <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="size-4 text-border-soft">
            <path d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" />
        </svg>
    </summary>

    <div class="absolute right-0 mt-2 w-48 rounded-xl bg-surface shadow-lg outline-1 outline-border-soft/30 py-2 z-10">
        <a href="#" class="flex items-center gap-3 px-4 py-2 text-sm text-ink hover:bg-accent/10">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" class="size-4 text-muted">
        ...
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="group w-full flex items-center gap-3 px-4 py-2 text-sm text-ink hover:bg-red-50 hover:text-red-600 transition-colors">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" class="size-4 text-muted group-hover:text-red-500">
